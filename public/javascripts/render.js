...
let renderHand = (handCards, flip = false) => { // Renders hand (for both self and enemy)
  ...
  let height = flip ? -100 : $(window).height() - 100; // -100 is to not show the whole cards and create more space
  let dangle = flip ? 1 : -1; // Rotation offset

  let i = 1,
      leftIndex = -1,
      rightIndex = -1,
      half = Math.floor(handCards.length / 2),
      offset = ($(window).width() / 2) - (35 * handCards.length / 2) - 50;

  if (handCards.length % 2 == 1) {
    leftIndex = half - 1;
    rightIndex = half + 1;
    setElementPos(handCards[half], $(window).width() / 2 - 70, height, half + 100, 0);
  } else {
    leftIndex = half - 1;
    rightIndex = half;
  }
  ...
  while (leftIndex >= 0) { // Start at middle card and setPos going outward
    setElementPos(handCards[leftIndex], offset + leftIndex * 35, height, leftIndex + 100, i * dangle);
    setElementPos(handCards[rightIndex], offset + rightIndex * 35, height, rightIndex + 100, i * -dangle);
    leftIndex--;
    rightIndex++;
    i++;
  }
...
//render the cards in the middle
